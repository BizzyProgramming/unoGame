...
public class UnoGameAPI {
	...
	public static UnoGameAPI getGame(String gameName) {
		...
		// Using with the Get Method in Controller
	...
	public boolean nextTurn() {
		...
		if (turn.isWinner() == false) { // loops through players until there is a winner
			...
			// turn.getHands().get(turn.getCurrentPlayer()));
			Cards card = turn.getHands().get(turn.getCurrentPlayer()).hasMatch(turn.getTopDiscard()); // checking if
																										// player has a
																										// match
			...
			if (card != null) { // if the player can play a card
				...
				turn.getDeck().addToDiscard(card); // takes the card the player played and puts it at the top of the
													// discard deck
				...
				turn.getHands().get(turn.getCurrentPlayer()).drawCard(turn.getDeck().dealCard()); // player draws one
																									// card
				// System.out.printf("Draw Card: %d \n", turn.getI()); //display player number
				// System.out.println(turn.getHands().get(turn.getCurrentPlayer()));
				// //displayers current players hand
			}
			if (!turn.isWinner()) {
